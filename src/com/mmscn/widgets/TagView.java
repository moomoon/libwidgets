...
public class TagView extends TextView {
	...
	public TagView(Context context, AttributeSet attrs, int defStyle) {
		...
		if (attrs != null) {
			TypedArray attributesArray = context.getTheme()
					.obtainStyledAttributes(attrs, R.styleable.TagView,
							defStyle, R.style.Widget_TagView);
			tagPadding = attributesArray.getDimensionPixelSize(
					R.styleable.TagView_tagPadding,
					dipToPixels(DEFAULT_PADDING));
			tagCornerRadius = attributesArray.getDimensionPixelSize(
					R.styleable.TagView_tagCornerRadius,
					dipToPixels(DEFAULT_CORNER_RADIUS));
			uppercaseTags = attributesArray.getBoolean(
					R.styleable.TagView_tagUppercase, DEFAULT_UPPERCASE);
	...
	public void setTags(List<? extends Tag> tags, String separator) {
		...
		while (it.hasNext()) {
			Tag tag = it.next();
			String tagContent = uppercaseTags ? tag.getTag().toUpperCase()
					: tag.getTag();
			sb.append(tagContent).setSpan(
					createSpan(tagContent, tag.getColor()),
					sb.length() - tagContent.length(), sb.length(),
	...
	private TagSpan createSpan(String text, int color) {
		return new TagSpan(text, tagPadding, getTextSize(),
				getTypeface() == Typeface.DEFAULT_BOLD, getCurrentTextColor(),
				color, tagCornerRadius);
	}
	...
	private static class TagSpan extends ImageSpan {
		public TagSpan(String text, int tagPadding, float textSize,
				boolean bold, int textColor, int tagColor,
				float roundCornersFactor) {
			super(new TagDrawable(text, tagPadding, textSize, bold, textColor,
					tagColor, roundCornersFactor));
		}
	}
	...
	private static class TagDrawable extends Drawable {
		...
		public TagDrawable(String text, int tagPadding, float textSize,
				boolean bold, int textColor, int tagColor,
				float roundCornersFactor) {
			this.backgroundPadding = new Rect(tagPadding, tagPadding,
			...
			setBounds(0, 0, (int) textContentPain.measureText(text)
					+ backgroundPadding.left + backgroundPadding.right,
					(int) (textContentPain.getTextSize()
		...
		public void draw(Canvas canvas) {
			canvas.drawRoundRect(fBounds, roundCornersFactor,
					roundCornersFactor, backgroundPaint);
			canvas.drawText(text, backgroundPadding.left + MAGIC_PADDING_LEFT,
					textContentPain.getTextSize() + backgroundPadding.top
							- MAGIC_PADDING_BOTTOM, textContentPain);
		}
